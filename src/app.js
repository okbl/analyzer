...
function savePrefs() {
  try { localStorage.setItem(PREFS_KEY, JSON.stringify({ filters, pdfOpts, crSort, allSort, allSet, hideFio })); }
...
function renderBento() {
  ...
  const rightTiles = [];
  rightTiles.push(`<div class="t link" data-tab="all" data-set="new">
  ...
  rightTiles.push(`<div class="t link" data-tab="all" data-set="nodata">
...
function wireBento() {
  const bento = $('bento');
  bento.querySelectorAll('[data-tab]').forEach((b) => b.addEventListener('click', () => {
    // Плитки открывают вкладку «Договоры» на нужном наборе: отдельных вкладок
    // под «новые» и «без данных» больше нет.
    if (b.dataset.set) { allSet = b.dataset.set; savePrefs(); renderPanels(); }
...
function selectTab(k) {
  // Набор вкладок зависит от отчёта (залоги есть не всегда), поэтому
  // несуществующую вкладку сводим к «Договорам», а не гасим все панели.
  activeTab = $('panel-' + k) ? k : 'all';
  renderTabs();
  const el = $('panel-' + activeTab);
...
function renderTabs() {
  ...
  const defs = [
    ...
    ['all', 'Договоры', report.contracts.length],
...
function renderPanels() {
  const deal = currentDeal();
  const res = deal && deal.date ? compute(deal) : null;
  renderOverdue(); renderContracts(res); renderPledges(); renderCheck();
}
...
function contractState(c) {
  if (c.closedDate) return `<span class="st-chip done">закрыт ${date(c.closedDate)}</span>`;
  if (c.section === 'closed' || c.isClosed) return '<span class="st-chip done">закрыт</span>';
  return '<span class="st-chip on">действующий</span>';
}

/*
 * Раньше «Новые договоры» и «Нет данных о платежах» были отдельными вкладками.
 * Это один и тот же список договоров, просто отфильтрованный, поэтому они
 * свелись в наборы этой вкладки: шесть вкладок в ряд читались хуже.
 */
function contractSets(res) {
  const all = report.contracts;
  const isNew = new Set(res ? res.newContracts : []);
  return [
    ['all', 'Все', all],
    ['open', 'Действующие', all.filter((c) => c.section !== 'closed')],
    ['closed', 'Закрытые', all.filter((c) => c.section === 'closed')],
    ['new', 'Новые после сделки', all.filter((c) => isNew.has(c))],
    ['nodata', 'Без данных о платежах', all.filter((c) => !c.hasPaymentTable)],
    ['pledge', 'С залогом', all.filter((c) => c.pledges.length)]
  ];
}

const SET_NOTES = {
  new: `<div class="note calm">Договоры, заключённые после указанной даты сделки. Определяется по полю
    «Дата совершения сделки». Исключить платежи по ним из расчёта можно галочкой в «Дополнительно».</div>`,
  nodata: `<div class="note"><b>Важно.</b> По этим договорам кредитор не передал в бюро построчный список
    платежей. Отсутствие платежей здесь <b>не значит, что их не было</b> — сведений просто нет в отчёте.
    В колонке «Снижение долга» — косвенная оценка по разделу «Сведения о сумме задолженности»:
    долг уменьшается не только от платежей, но и при списании, переуступке или реструктуризации.
    В расчёт она не входит.</div>`,
  pledge: `<div class="note calm">Договоры, обеспеченные залогом. Что именно в залоге и снят ли он —
    во вкладке «Залоги».</div>`
};
...
function renderContracts(res) {
  ...
  const sets = contractSets(res);
  let cur = sets.find((s) => s[0] === allSet);
  if (!cur || !cur[2].length) { cur = sets[0]; allSet = 'all'; }
  const list = cur[2];
  const from = res && res.deal ? res.deal.date : null;
  const creditors = new Set(list.map((c) => c.creditor));
  ...
  const lastCol = allSet === 'nodata'
    ? { head: 'Снижение долга', cell: (c) => {
      const est = P.principalRepaidSince(c.debtSnapshots, from);
      return c.debtSnapshots.length
        ? `<span${est > 0 ? ' style="font-weight:600"' : ' class="sub"'}>${money0(est)}</span>` : '—';
    } }
    : { head: 'Платежей', cell: (c) => c.hasPaymentTable ? c.payments.length : '<span class="sub">нет данных</span>' };

  box.innerHTML = `<div class="setbar">${sets.map(([k, label, arr]) =>
    `<button class="set${k === allSet ? ' on' : ''}${arr.length ? '' : ' empty'}" data-set="${k}"${arr.length ? '' : ' disabled'}>${label} <span class="n">${arr.length}</span></button>`).join('')}</div>
    ${SET_NOTES[allSet] || ''}
    <div class="sumline">
      <span><b>${creditors.size}</b> ${plural(creditors.size, 'кредитор', 'кредитора', 'кредиторов')}</span>
      <span><b>${list.length}</b> ${plural(list.length, 'договор', 'договора', 'договоров')}</span>
      <span class="sp"></span>
    ...
    <div class="card"><div class="scroll-x"><table class="rowopen">
      <thead><tr><th>Кредитор</th><th>Вид займа</th><th>Взят</th><th>Состояние</th>
        <th class="r">Сумма обязательства</th><th class="r">${lastCol.head}</th><th class="r">Лист</th></tr></thead>
      <tbody>${sorted.map((c, i) => {
    const est = c.debtSnapshots.length ? P.principalRepaidSince(c.debtSnapshots, from) : null;
    return `<tr class="main" data-row="${i}" tabindex="0">
        <td><b>${esc(c.creditor)}</b>${c.pledges.length ? ' <span class="pl-mark">залог</span>' : ''}
        ...
        <td class="r">${lastCol.cell(c)}</td>
        <td class="r sub">${c.page}<span class="caret2"></span></td></tr>
      <tr class="det" data-det="${i}" hidden><td colspan="7"><dl class="det-dl">
        <dt>Срок по договору</dt><dd>${planDate(c.plannedEnd)}</dd>
        ${c.closeReason ? `<dt>Основание закрытия</dt><dd>${esc(c.closeReason)}</dd>` : ''}
        ${c.obligationDate ? `<dt>Обязательство с</dt><dd>${date(c.obligationDate)}</dd>` : ''}
        ${c.participation ? `<dt>Вид участия</dt><dd>${esc(c.participation)}</dd>` : ''}
        ${c.controlTotals ? `<dt>Агрегат отчёта</dt><dd>${money(c.controlTotals.total)}</dd>` : ''}
        ${est != null && allSet !== 'nodata' ? `<dt>Снижение долга по снимкам</dt><dd>${money0(est)}</dd>` : ''}
        ${c.pledges.length ? `<dt>Залог</dt><dd>${c.pledges.map((p) =>
      `${esc(p.subject || 'предмет не назван')} — ${p.released ? 'снят ' + date(p.endActual) : 'в силе'}`).join('; ')}</dd>` : ''}
        ${c.statuses.length ? `<dt>Отметки отчёта</dt><dd class="sub">${c.statuses.map(esc).join(' · ')}</dd>` : ''}
      </dl></td></tr>`;
  }).join('')}</tbody></table></div></div>
    <p class="hint">Строка раскрывается по клику: срок по договору, основание закрытия и прочие поля карточки.
      «Взят» — поле «Дата совершения сделки». «Состояние» с датой берётся из «Даты фактического прекращения
      обязательства»; без даты показан раздел отчёта, в котором напечатан договор. Номер после кредитора —
      нумерация самого отчёта, своя в разделах действующих и закрытых. Просрочки — во вкладке «Просрочка».</p>`;

  const sel = $('all-sort');
  if (sel) sel.addEventListener('change', (e) => { allSort = e.target.value; savePrefs(); renderContracts(res); });
  box.querySelectorAll('.setbar .set').forEach((b) => b.addEventListener('click', () => {
    allSet = b.dataset.set; savePrefs(); renderContracts(res);
  }));
  const toggle = (tr) => {
    const det = box.querySelector(`tr.det[data-det="${tr.dataset.row}"]`);
    if (!det) return;
    det.hidden = !det.hidden;
    tr.classList.toggle('open', !det.hidden);
  };
  box.querySelectorAll('tr.main').forEach((tr) => {
...
function renderPledges() {
  ...
  box.innerHTML = `<div class="note calm">Кредиторы, у которых обязательство обеспечено залогом.
    ...
    ${rows.map(({ c, p }) => {
    // На виду только то, ради чего открывают вкладку: что в залоге, сколько
    // стоит и снят ли. Реквизиты — под «подробнее».
    const more = [
      p.code ? ['Идентификатор', `<span class="mono">${esc(p.code)}</span>`] : null,
      p.currentValue != null ? ['Актуальная стоимость',
        money0(p.currentValue) + (p.currentDate ? ` <span class="sub">на ${date(p.currentDate)}</span>` : '')] : null,
      p.securedTotal != null ? ['Обеспечено обязательств', money0(p.securedTotal) +
        (p.securedCount ? ` <span class="sub">по ${p.securedCount} ${plural(p.securedCount, 'договору', 'договорам', 'договорам')}</span>` : '')] : null,
      ['Срок по договору залога', `<span class="sub">${planDate(p.endPlanned)}</span>`],
      p.released && p.endReason ? ['Причина снятия', `<span class="sub">${esc(p.endReason.toLowerCase())}</span>`] : null,
      p.place ? ['Место нахождения', `<span class="sub">${esc(p.place)}</span>`] : null
    ].filter(Boolean);

    return `<div class="pl-card${p.released ? ' done' : ''}">
      <div class="pl-head">
        <div>
          <b>${esc(c.creditor)}</b>
      ...
      ${more.length ? `<details class="pl-more"><summary><span class="caret"></span>Подробнее</summary>
        <dl class="pl-dl">${more.map(([k, v]) => `<dt>${k}</dt><dd>${v}</dd>`).join('')}</dl></details>` : ''}
